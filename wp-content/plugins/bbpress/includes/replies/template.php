...
function bbp_get_reply_author_avatar($reply_id = 0, $size = 40) {
    ...
    if ($user_photo_nm) {
        ...
        $author_avatar = '<img src="' . $img_url . '" class="avatar avatar-' . $size . ' photo" style="height:' . $size . 'px; width:' . $size . 'px"/>';
    } else {
        if (!empty($reply_id)) {
...
function bbp_get_reply_author_link($args = '') {
    ...
    if (!empty($reply_id)) {
        ...
        $author_url = get_author_posts_url($user_id); //bbp_get_topic_author_url($topic_id);
...
function bbp_get_reply_author_role($args = array()) {
    ...
    $role = 'Registered VAR Member'; bbp_get_user_display_role(bbp_get_reply_author_id($reply_id));
