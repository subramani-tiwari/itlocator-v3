...
function make_marker(){
    ...
    if( $userdata != null ){
        ...
        for ( var i = 0; i < $userdata.length; i++ ){
            ...
            if($userdata[i].user_role >= 1 && $userdata[i].logo_url) {
                info_content += '<img src="'+ $userdata[i].logo_url+'" width="90" class="pull-left" style="margin-right: 5px; margin-bottom: 10px;display: block;"/>';
                info_content += '<a href="'+$userdata[i].permalink+'" class="font-color-fff text-decoration-none" style="display: block;"><h4 class="font-size-14">'+$userdata[i].name+'</h4><div class="font-size-14">' + $userdata[i].id + '</div><div class="font-size-14" style="width: 310px; display: block;">';
            } else {
                info_content += '<a href="'+$userdata[i].permalink+'" class="font-color-fff text-decoration-none" style="display: block;"><h4 class="font-size-14">'+$userdata[i].name+'</h4><div class="font-size-14">' + $userdata[i].id + '</div><div class="font-size-14" style="width: 200px; display: block;">';
            }
...
function make_marker_no_desc(x,y) {
    $idx = 0;
    $x = new Array();
    $y = new Array();
    $x[$idx] = x;
    ...
    if($userdata != null) {
        for (var i = 0; i < $userdata.length; i++) {
            ...
            $markers[$idx] = new google.maps.Marker({
                position: new google.maps.LatLng($userdata[i].x, $userdata[i].y),
                map: $map,
                icon : $icon_url,
                title: $userdata[i].address
            });
        ...
        if(jQuery(this).val()) {
            geocoder.geocode({
                ...
                if (status == google.maps.GeocoderStatus.OK) {
                    ...
                    lat_a[lat_a.length] = lat;
                    lng_a[lng_a.length] = lng;
                    ...
                    $markers[$idx] = new google.maps.Marker({
                        ...
                        map: $map,
                        icon : $icon_url,
                        title: jQuery('.locations', jQuery(this).parent()).val()
                    });
                    jQuery('.locations').each(function (i) {
                        jQuery('.locations_lat', jQuery(this).parent()).val(lat_a[i]);
                        jQuery('.locations_lng', jQuery(this).parent()).val(lng_a[i]);
                        if($root_val) {
                            if($root_val == jQuery(this).val())
                                $map.setCenter(center);
                        } else {
                            $map.setCenter(center);
                        }
 ...
 */
    ...
    jQuery(".setting_lat_lng").live('click', function() {
        if(jQuery('.locations', jQuery(this).parent()).val()) {
            ...
            $root_val = jQuery('.locations', jQuery(this).parent()).val();
            reflesh_map_by_location ($icon_url, $root_val);
        }
    });  
